<template>
  ...
  <div class="notice">
    ...
    <el-card>
      ...
      <el-button size="small" type="danger" @click="exportXlsxPopulate()">用xlsx-populate导出Excel</el-button>
...
<script>
import { get_notice_list, add_notice, generate_excel } from '../api/property/notice'
...
export default {
  ...
  methods: {
    ...
    exportXlsxPopulate() {
      ...
      exportXlsxPopulate(this.tableData, '111', this) // 传入的参数分别为：数据，表格的名称，这个vue组件的实例this
    },
    // 样式暂时没有开启
    // exportXlsxPopulate(tableData, title = '表格', vm) {
    ...
    //     const COLUMN = Object.keys(tableData.data[0])
    //     COLUMN.forEach((item) => {
    //       // 每一列 wb.column('A').width(10)
    //       wb.column(String.fromCharCode(charcode_a_uc)).width(Number(String(tableData.data[0][item]).length) + 5)
    //       // 表头 wb.cell('A3').value('编号ID')
    //       wb.cell(String.fromCharCode(charcode_a_uc) + count).value(item)
    //       let temp_num = count + 1
    //       tableData.data.forEach((i) => {
    //         // 数据 wb.cell('A' + count).value(i.id)
